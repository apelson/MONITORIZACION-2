    ...
    <div className="login-page" data-testid="login-page">
      ...
      <div className="login-container">
        ...
        <div className="login-client-brand">
          <div className="login-dag-text">
            <span className="dag-name">DOMINGO ALONSO</span>
            <span className="dag-group">GROUP</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="login-form" data-testid="login-form">
          <div className="login-form-title">
            <Lock size={15} className="login-form-icon"/>
          ...
          <div className="login-field">
            ...
            <div className="login-input-box">
              ...
              <input data-testid="login-username" type="text" value={username}
                onChange={e=>{setUsername(e.target.value);setError('');}} placeholder="Introduce tu usuario"/>
            </div>
          </div>
          <div className="login-field">
            ...
            <div className="login-input-box">
              ...
              <input data-testid="login-password" type={showPw?'text':'password'} value={password}
                onChange={e=>{setPassword(e.target.value);setError('');}} placeholder="Introduce tu contrasena"/>
          ...
          <button data-testid="login-submit" type="submit" className="login-submit-btn" disabled={loading}>
  ...
  return (
    <div className="app-shell" data-testid="dashboard">
      ...
      <header className="app-header">
        <div className="header-left">
          <div className="header-brand">
            <span className="header-dag">DOMINGO ALONSO</span>
            <span className="header-dag-sub">GROUP</span>
          </div>
          <div className="header-divider"/>
          <div className="header-app-info">
            <h1 className="header-app-name">Conteo de Visitas</h1>
            <span className="header-app-tagline">Tiempo Real</span>
          </div>
        </div>
        <div className="header-center">
          <LiveClock compact/>
          <button className="header-icon-btn" onClick={()=>{setLoading(true);fetchData();}} data-testid="refresh-btn" title="Actualizar">
            <RefreshCw size={14} className={loading?'spin':''}/>
          </button>
          <button className={`header-icon-btn ${autoRefresh?'active':''}`} onClick={()=>setAutoRefresh(!autoRefresh)} data-testid="auto-refresh-btn" title="Auto-refresh">
            {autoRefresh?<Wifi size={14}/>:<WifiOff size={14}/>}
          </button>
          {view==='noc'&&<button className="header-icon-btn active" onClick={()=>setNocFs(true)} data-testid="fullscreen-btn" title="Pantalla completa"><Maximize2 size={14}/></button>}
        </div>
        <div className="header-right">
          ...
          <button className="header-logout" onClick={onLogout} data-testid="logout-btn" title="Cerrar sesion"><LogOut size={16}/></button>
        </div>
      </header>
      ...
      <nav className="app-nav">
        {navItems.map(item => (
          <button key={item.id} className={`app-nav-item ${view===item.id?'active':''}`}
  ...
  return (
    <div className="view-wrap" data-testid="realtime-view">
      <div className="kpi-row">
        <KpiCard label="Visitas Hoy" value={totals.entries||0} icon={Users} accent="#00b4d8"/>
        <KpiCard label="Camaras Online" value={`${cameras_online}/${cameras_total}`} icon={Camera} accent="#22c55e"/>
      </div>
      <div className="card">
        ...
        <div className="rank-list">
          ...
          {ranking.map((item,i) => (
            <div key={item.brand_id} className="rank-row" data-testid={`ranking-item-${item.brand_id}`}>
              ...
              <div className="rank-logo"><BrandLogo brandId={item.brand_id} size={28}/></div>
            ...
            {ranking.flatMap(brand=>(brand.cameras||[]).map(cam=>(
              <div key={cam.camera_id} className="cam-card">
                <div className="cam-card-top">
                  <span className={`cam-dot ${cam.status}`}/><span className="cam-card-name">{cam.camera_name}</span>
                </div>
      ...
      <div className="card">
        ...
        <div className="rank-list">
          ...
          {ranking.map((item,i) => {
            ...
            const color = BRAND_COLORS[item.brand_id] || item.brand_color || '#00b4d8';
            return (
              <div key={item.brand_id} className="brand-row" data-testid={`brand-row-${item.brand_id}`}>
                <div className="brand-row-left">
                  ...
                  <div className="brand-row-logo"><BrandLogo brandId={item.brand_id} size={32}/></div>
        ...
        <div className="rank-list">
          ...
          {ranking.map((item,i) => (
            <div key={item.center_id} className="rank-row" data-testid={`center-item-${item.center_id}`}>
              <div className={`rank-pos ${i<3?`medal-${i+1}`:''}`}>{i+1}</div>
              <div className="rank-logo"><BrandLogo brandId={item.brand_id} size={22}/></div>
  ...
  return (
    <div className="view-wrap" data-testid="cameras-view">
      {isAdmin&&<div className="action-bar">
        ...
        <button className="btn-outline" onClick={handleMigrate} disabled={migrating} data-testid="migrate-cameras-btn">{migrating?<RefreshCw size={16} className="spin"/>:<Download size={16}/>}<span>{migrating?'Migrando...':'Importar de Plataforma'}</span></button>
      </div>}
      ...
      {showForm&&<Modal title={editCam?'Editar Camara':'Nueva Camara'} onClose={()=>setShowForm(false)}>
        <div className="form-2col">
          ...
          <FormField label="Nombre *" value={form.camera_name} onChange={v=>setForm({...form,camera_name:v})} placeholder="ej: AUDI Tenerife"/>
      ...
      <div className="card">
        <div className="card-header-row"><h2 className="card-title"><Camera size={18}/> Camaras Configuradas</h2><span className="count-badge">{cameras.length}</span></div>
        {cameras.length===0?<EmptyState text="No hay camaras configuradas"/>:(
          <div className="table-wrap"><table className="data-table"><thead><tr><th>ID</th><th>Nombre</th><th>Marca</th><th>Isla</th><th>IP:Puerto</th><th>Estado</th>{isAdmin&&<th>Acciones</th>}</tr></thead><tbody>
            {cameras.map(cam=>(<tr key={cam.camera_id} data-testid={`camera-row-${cam.camera_id}`}>
              <td className="mono">{cam.camera_id}</td><td>{cam.camera_name}</td>
              <td><div className="table-brand"><BrandLogo brandId={cam.brand_id} size={16}/><span>{cam.brand_id}</span></div></td>
              <td className="capitalize">{cam.island?.replace(/-/g,' ')}</td><td className="mono">{cam.ip}:{cam.port}</td>
              <td><span className={`status-pill ${cam.enabled!==false?'on':'off'}`}>{cam.enabled!==false?'Activa':'Inactiva'}</span></td>
              {isAdmin&&<td><div className="table-actions"><button className="tbl-btn" onClick={()=>openEdit(cam)} data-testid={`edit-camera-${cam.camera_id}`}><Edit3 size={13}/></button><button className="tbl-btn danger" onClick={()=>handleDelete(cam.camera_id)} data-testid={`delete-camera-${cam.camera_id}`}><Trash2 size={13}/></button></div></td>}
            </tr>))}
          </tbody></table></div>
        )}
      </div>
...
function UsersView({ data, api, onRefresh, currentUser }) {
  ...
  const handleSave = async () => {
    try {
      if (editUser) { const body = {full_name:form.full_name,role:form.role}; if(form.password)body.password=form.password; await api('put',`/users/${editUser}`,body); }
  ...
  return (
    <div className="view-wrap" data-testid="users-view">
      ...
      <div className="card">
        <div className="card-header-row"><h2 className="card-title"><UserCog size={18}/> Usuarios del Sistema</h2><span className="count-badge">{users.length}</span></div>
            ...
            return (
              <div key={u.id} className={`user-card ${!isActive?'dimmed':''}`} data-testid={`user-card-${u.username}`}>
                <div className="user-card-top">
                  <div className="user-card-avatar" style={{background:u.role==='admin'?'linear-gradient(135deg,#f59e0b,#d97706)':'linear-gradient(135deg,#00b4d8,#0077b6)'}}>
                    {(u.full_name||u.username)[0].toUpperCase()}
                  </div>
                  <div className="user-card-info"><span className="user-card-name">{u.full_name||u.username}</span><span className="user-card-handle">@{u.username}</span></div>
                  {u.id!==currentUser?.id&&(
                    <div className="user-card-btns">
                      <button className="tbl-btn" onClick={()=>openEdit(u)} title="Editar" data-testid={`edit-user-${u.username}`}><Edit3 size={13}/></button>
                      <button className="tbl-btn" onClick={()=>handleToggle(u)} title={isActive?'Desactivar':'Activar'} data-testid={`toggle-user-${u.username}`}>
                        {isActive?<ToggleRight size={14} style={{color:'#22c55e'}}/>:<ToggleLeft size={14} style={{color:'#ef4444'}}/>}
                      </button>
                      <button className="tbl-btn danger" onClick={()=>handleDelete(u.id,u.username)} title="Eliminar" data-testid={`delete-user-${u.username}`}><Trash2 size={13}/></button>
                    </div>
                  )}
                </div>
      ...
      <div className="noc-inner">
        ...
        <header className="noc-header">
          <div className="noc-header-left">
            <div className="noc-dag-brand"><span className="noc-dag-name">DOMINGO ALONSO</span><span className="noc-dag-sub">GROUP</span></div>
        ...
        <div className="noc-body">
          <div className="noc-col-left">
            ...
            <div className="noc-panel noc-podium-panel">
              <div className="noc-panel-title"><Award size={15} className="gold-icon"/> Podio de Honor</div>
              <div className="noc-podium">
                {[1,0,2].map(idx => {
                  const item = ranking[idx]; if(!item) return <div key={idx} className="podium-slot empty"/>;
                  const r = idx+1; const h = {1:130,2:100,3:75}; const ord = idx===0?2:idx===1?1:3;
                  return (
                    <div key={idx} className={`podium-slot r-${r}`} style={{order:ord}} data-testid={`podium-rank-${r}`}>
                      {r===1&&<Crown size={22} className="podium-crown"/>}
                      <div className="podium-logo"><BrandLogo brandId={item.brand_id} size={r===1?44:34}/></div>
            ...
            {/* Full ranking */}
            <div className="noc-panel noc-ranking-panel">
              <div className="noc-panel-title"><BarChart3 size={15}/> Ranking Completo</div>
              <div className="noc-rank-rows">
                {ranking.slice(0,8).map((item,i) => (
                  <div key={item.brand_id} className={`noc-rank-row ${i===0?'leader':''}`} data-testid={`noc-rank-${item.brand_id}`}>
                    ...
                    <BrandLogo brandId={item.brand_id} size={20}/>
                ...
                {ranking.length===0&&<div className="noc-empty"><Trophy size={28}/><p>Esperando datos...</p></div>}
              </div>
            </div>
          </div>
          ...
          <div className="noc-col-right">
            ...
            <div className="noc-islands">
              {ISLANDS_CFG.map(island => {
                ...
                return (
                  <div key={island.id} className={`noc-island ${isLeader?'island-leader':''} ${stats.total===0?'island-zero':''}`} data-testid={`island-${island.id}`}>
                    {isLeader&&<Crown size={13} className="island-crown-ico"/>}
                    <div className="island-visual">
                      <IslandSilhouette island={island.id} size={60}/>
                    ...
                    <div className="island-data">
                      ...
                      <span className={`island-total mono ${isLeader?'gold':''}`}><AnimNum value={stats.total||0}/></span>
                    </div>
            ...
            <div className="noc-panel noc-summary-panel">
        ...
        <footer className="noc-footer">
          ...
          <div className="noc-foot-right"><span>Desarrollado por</span><span className="noc-foot-brand">Siempria</span><span className="noc-foot-sep">|</span><span>Tecnologia Mobotix</span></div>
        </footer>
      </div>
    </div>
  );
...
function KpiCard({ label, value, icon: Icon, accent }) {
  return (
    <div className="kpi-card" data-testid={`stat-${label.toLowerCase().replace(/\s+/g,'-')}`}>
      <div className="kpi-icon" style={{color:accent,background:`${accent}15`}}><Icon size={20}/></div>
      <div className="kpi-data"><span className="kpi-val mono">{typeof value==='number'?value.toLocaleString('es-ES'):value}</span><span className="kpi-label">{label}</span></div>
    </div>
  );
}
...
function Modal({ title, onClose, children }) {
  return <div className="modal-bg" onClick={onClose} data-testid="modal-overlay"><div className="modal-box" onClick={e=>e.stopPropagation()}>
    <div className="modal-top"><h3>{title}</h3><button onClick={onClose} data-testid="modal-close"><X size={18}/></button></div>
    <div className="modal-inner">{children}</div>
  </div></div>;
}
function FormField({ label, value, onChange, placeholder, disabled, type='text', testId }) {
  return <div className="form-field"><label>{label}</label><input data-testid={testId} type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} disabled={disabled}/></div>;
}
function FormSelect({ label, value, onChange, options, testId }) {
  return <div className="form-field"><label>{label}</label><select data-testid={testId} value={value} onChange={e=>onChange(e.target.value)}><option value="">Seleccionar...</option>{options.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select></div>;
}
