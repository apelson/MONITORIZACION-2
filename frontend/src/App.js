...
function Dashboard({ token, user, onLogout }) {
  ...
  return (
    <div className="conteo-dashboard" data-testid="dashboard">
      <header className="dash-header">
        ...
        <div className="dash-header-center">
          <LiveClock compact />
          <button className="header-refresh" onClick={()=>{setLoading(true);fetchData();}} data-testid="refresh-btn"><RefreshCw size={14} className={loading?'spin':''}/></button>
          <button className={`header-auto ${autoRefresh?'active':''}`} onClick={()=>setAutoRefresh(!autoRefresh)} data-testid="auto-refresh-btn">{autoRefresh?<Wifi size={14}/>:<WifiOff size={14}/>}</button>
          {view==='noc'&&<button className="header-auto active" onClick={()=>setNocFs(true)} data-testid="fullscreen-btn"><Maximize2 size={14}/></button>}
        </div>
    ...
    <div className="view-container" data-testid="realtime-view">
      ...
      <div className="section-card">
        ...
        <div className="ranking-list">
          {ranking.length===0&&<EmptyState text="Sin datos de camaras - Las camaras estan en la red local del servidor"/>}
          {ranking.map((item,idx) => (
            <div key={item.brand_id} className="ranking-item" data-testid={`ranking-item-${item.brand_id}`}>
...
function BrandRankingView({ data }) {
  if (!data) return <EmptyState text="Sin datos" />;
  const { ranking=[] } = data;
  const maxVal = Math.max(...ranking.map(r=>r.total_visits||0),1);
  return (
    <div className="view-container" data-testid="brand-ranking-view">
      <div className="section-card">
        ...
        <div className="ranking-list">
          {ranking.length===0&&<EmptyState text="Sin datos de camaras - Las camaras estan en la red local del servidor"/>}
          {ranking.map((item,idx) => (
            <div key={item.brand_id} className="ranking-row" data-testid={`brand-row-${item.brand_id}`}>
              <div className="rank-pos">#{idx+1}</div>
              <BrandLogo brandId={item.brand_id} size={32}/>
              <div className="rank-info">
                <span className="rank-name">{item.brand_name}</span>
                <div className="rank-bar-wrap"><div className="rank-bar" style={{width:`${((item.total_visits||0)/maxVal)*100}%`,background:item.brand_color||'#00AEEF'}}/></div>
              </div>
              <div className="rank-value"><AnimNum value={item.total_visits||0}/></div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="view-container" data-testid="center-ranking-view">
      <div className="section-card">
        ...
        <div className="ranking-list">
          {ranking.length===0&&<EmptyState text="Sin datos de centros - Las camaras estan en la red local del servidor"/>}
          {ranking.map((item,idx) => (
            <div key={item.center_id} className="ranking-item" data-testid={`center-item-${item.center_id}`}>
              <div className={`rank-position ${idx<3?`top-${idx+1}`:''}`}>{idx+1}</div>
  ...
  return (
    <div className="view-container" data-testid="cameras-view">
      {isAdmin&&<div className="cameras-actions">
        ...
        <button className="btn-secondary" onClick={handleMigrate} disabled={migrating} data-testid="migrate-cameras-btn">{migrating?<RefreshCw size={16} className="spin"/>:<Download size={16}/>}<span>{migrating?'Migrando...':'Importar de Plataforma Principal'}</span></button>
      </div>}
      ...
      {showForm&&<Modal title={editCam?'Editar Camara':'Nueva Camara'} onClose={()=>setShowForm(false)}>
        ...
        <div className="modal-actions"><button className="btn-ghost" onClick={()=>setShowForm(false)}>Cancelar</button><button className="btn-primary" onClick={handleSave} data-testid="save-camera-btn"><Save size={16}/><span>Guardar</span></button></div>
      </Modal>}
      <div className="section-card">
  ...
  return (
    <div className="view-container" data-testid="users-view">
      ...
      {showForm&&<Modal title={editUser?'Editar Usuario':'Nuevo Usuario'} onClose={()=>setShowForm(false)}>
        <div className="form-grid">
          <div className="field"><label>Nombre de usuario *</label><input data-testid="user-username-input" value={form.username} onChange={e=>setForm({...form,username:e.target.value})} placeholder="usuario" disabled={!!editUser}/></div>
          <div className="field"><label>Nombre completo</label><input data-testid="user-fullname-input" value={form.full_name} onChange={e=>setForm({...form,full_name:e.target.value})} placeholder="Juan Garcia"/></div>
          <div className="field"><label>{editUser?'Nueva contrasena (dejar vacio para mantener)':'Contrasena *'}</label><input data-testid="user-password-input" type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder="****"/></div>
          <div className="field"><label>Rol *</label><select data-testid="user-role-select" value={form.role} onChange={e=>setForm({...form,role:e.target.value})}>{roles.map(r=><option key={r.id} value={r.id}>{r.name} - {r.desc}</option>)}</select></div>
        </div>
        <div className="modal-actions"><button className="btn-ghost" onClick={()=>setShowForm(false)}>Cancelar</button><button className="btn-primary" onClick={handleSave} data-testid="save-user-btn"><Save size={16}/><span>Guardar</span></button></div>
      </Modal>}
      ...
      <div className="section-card">
        ...
        <div className="users-grid">
          {users.map(u => (
            <div key={u.id} className={`user-card ${!u.is_active?'inactive':''}`} data-testid={`user-card-${u.username}`}>
              <div className="user-card-header">
                ...
                {u.id !== currentUser?.id && (
                  <div className="user-card-actions">
                    <button className="icon-btn" onClick={()=>openEdit(u)} title="Editar" data-testid={`edit-user-${u.username}`}><Edit3 size={14}/></button>
                    <button className="icon-btn" onClick={()=>handleToggle(u)} title={u.is_active?'Desactivar':'Activar'} data-testid={`toggle-user-${u.username}`}>{u.is_active?<ToggleRight size={14} className="text-green"/>:<ToggleLeft size={14} className="text-red"/>}</button>
                    <button className="icon-btn danger" onClick={()=>handleDelete(u.id,u.username)} title="Eliminar" data-testid={`delete-user-${u.username}`}><Trash2 size={14}/></button>
                  </div>
                )}
              </div>
      ...
      <div className="noc-content">
        <header className="noc-header">
          ...
          <div className="noc-header-right">
            ...
            <button className={`noc-btn ${autoRefresh?'active':''}`} onClick={()=>setAutoRefresh(!autoRefresh)} data-testid="noc-auto-refresh"><RefreshCw size={14} className={loading?'spin':''}/><span>{autoRefresh?'30s':'Off'}</span></button>
            {onClose&&<button className="noc-btn-close" onClick={onClose} data-testid="noc-close"><X size={18}/></button>}
          </div>
        </header>
            ...
            <div className="noc-section noc-podium-section">
              ...
              <div className="noc-podium">
                {[1,0,2].map(idx => {
                  ...
                  return (
                    <div key={idx} className={`podium-slot rank-${rank}`} style={{order}} data-testid={`podium-rank-${rank}`}>
          ...
          <div className="noc-right">
            ...
            <div className="noc-islands-grid">
              {ISLANDS_CFG.map(island => {
                ...
                return (
                  <div key={island.id} className={`noc-island-card ${isLeader?'leader':''} ${stats.total===0?'inactive':''}`} data-testid={`island-${island.id}`}>
...
function StatCard({ label, value, icon: Icon, color }) {
  return <div className="stat-card" data-testid={`stat-${label.toLowerCase().replace(/\s+/g,'-')}`}><div className="stat-icon" style={{color,background:`${color}15`}}><Icon size={20}/></div><div className="stat-info"><span className="stat-value">{typeof value==='number'?value.toLocaleString():value}</span><span className="stat-label">{label}</span></div></div>;
}
function EmptyState({ text }) { return <div className="empty-state" data-testid="empty-state"><BarChart3 size={36}/><p>{text}</p></div>; }
function Modal({ title, onClose, children }) {
  return <div className="modal-overlay" onClick={onClose} data-testid="modal-overlay"><div className="modal-content" onClick={e=>e.stopPropagation()}>
    <div className="modal-header"><h3>{title}</h3><button onClick={onClose} data-testid="modal-close"><X size={18}/></button></div>
